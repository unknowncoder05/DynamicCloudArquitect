...
{{app_name}} uses SQLite as its database with **S3-based persistence** for the on-demand backend architecture. The database is synchronized between the ECS container and S3 bucket on startup and shutdown.
...
```json
{
  ...
  "Resource": [
    "arn:aws:s3:::{{app_name}}-database-prod",
    "arn:aws:s3:::{{app_name}}-database-prod/*"
  ]
}
```
...
aws s3 cp s3://{{app_name}}-database-prod/db.sqlite3 ./db.sqlite3
```
...
# Or directly with AWS CLI
aws s3 cp ./db.sqlite3 s3://{{app_name}}-database-prod/db.sqlite3
```

**Restore from Backup:**
```bash
# List available backups
aws s3 ls s3://{{app_name}}-database-prod/ | grep backup

# Restore specific backup
aws s3 cp s3://{{app_name}}-database-prod/db.sqlite3.backup.20231202_143022 \
          s3://{{app_name}}-database-prod/db.sqlite3
```
...
```bash
aws logs tail /ecs/{{app_name}}-prod --follow --filter-pattern "[INFO]"
```
...
**List all backups:**
```bash
aws s3 ls s3://{{app_name}}-database-prod/ \
  | grep backup \
  | sort -r
```
...
aws s3 cp s3://{{app_name}}-database-prod/db.sqlite3 /tmp/test.db
...
aws s3 cp s3://{{app_name}}-database-prod/db.sqlite3.backup.TIMESTAMP \
          ./db-good.sqlite3
...
aws s3 cp test.db s3://{{app_name}}-database-prod/db.sqlite3
...
# 4. Check S3
aws s3 cp s3://{{app_name}}-database-prod/db.sqlite3 ./test-downloaded.db
